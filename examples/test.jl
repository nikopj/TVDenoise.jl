...
# 	x, hist = tvd_pds(video_n[i,:,:,:], 0.2, 0.5,0.5; isotropic=true, maxit=100, tol=1e-3, verbose=false)
# 	video_dn[i,:,:,:,1] = x
# end
# println("\nDone.")

video_dn, hist = tvd_pds(video_n[1:10,:,:,:], 0.2, 0.1, 0.1; isotropic=true, maxit=50, tol=1e-3, verbose=true)

for i ∈ 1:size(video_dn[1])
